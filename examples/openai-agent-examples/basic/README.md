...
**Port:** One `agskill` with `system_prompt`, `replace_tools=[]`. A single `ag.run()` call blocks until the skill resolves.
...
**Port:** `agtool(name, description, fn, params)` replaces `@function_tool`. The `fn` receives an `agdata` (from the LLM's JSON arguments) and returns an `agdata`. The skill's `add_tools=[get_weather]` adds the weather tool on top of any agent-level tools already available.
