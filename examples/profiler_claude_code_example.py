"""
Profiler example (2 of 2): full-coverage profiling of the **claude_code** engine.

This file and ``profiler_native_example.py`` are deliberately the same script
-- same skills, same schemas, same phases, same report -- differing only in
``ENGINE``. Run both against the same model and diff their ``summary.json`` to
...
Anthropic credentials, since agProxyLLM swaps them for a per-run gateway token
pointing back at the agllm backend configured below.
...
Every section ``summary.json`` can produce, and the phase that fills it:
  ...
  tool_metrics       all phases -- ``tool:{name}`` (see "Coverage" below)
  ...
  process_metrics    phase 1-2 -- per-PID /proc sampling inside the workload
                     ...
                     records from execve/exit syscall events. This is the one
                     place an external CLI's internals become visible without
                     its cooperation: every `node`, `rg`, `bash` the harness
                     spawns shows up here
...
Coverage: how this differs from native
--------------------------------------
The harness abstraction moved the ReAct loop out of the host process, so turn
and tool structure is not free. Claude Code reaches exact coverage by a
different route than native's in-container emitter -- its ``PreToolUse`` /
``PostToolUse`` hooks POST to the profiler ingest, which mints one exact
``tool:{name}`` span per ``tool_use_id``. Underneath that, turn and tool
boundaries are *also* derivable host-side for every engine by diffing the
conversation each dispatch resends to the terminus, tagged
``metadata["timing"] = "derived"``. Two things follow when diffing against the
native run:

  * **Tool names differ in case.** Claude Code reports its own tool
    vocabulary (``tool:Bash``, ``tool:Read``, ``tool:Write``); native reports
    ``tool:bash``, ``tool:read``, ``tool:write``. The prompts below are
    identical on purpose -- do not normalise one to the other.
  * **Compaction and harness-internal retries are unavailable.** Claude Code
    compacts inside itself and retries inside itself; a CLI-level retry
    arrives at the terminus as a fresh dispatch. ``llm_metrics['attempts']``
...
Two consequences worth internalising:
  ...
  * Only egress *the sandbox itself performs* shows up. That is why phase 1
    tells the agent to fetch over ``bash``. A host-side fetch tool would
    satisfy the same prompt while bypassing the container's network namespace
...
no output and no measurable overhead. Two ways to turn it on:
    ...
    AGENCY_PROFILE=1 AGENCY_PROFILE_DIR=./runs/profile \
        .venv/bin/python3 examples/profiler_claude_code_example.py
...
Requirements: **Linux only** (cgroups v2 + /proc) and the `claude` CLI on
PATH. opentelemetry-sdk/-exporter and nvidia-ml-py are core dependencies, so
a plain ``uv pip install -e .`` already has everything this needs.
...
def _print_profile_report(profile_dir: Path) -> None:
    ...
    # `attempts` is host truth from the terminus. Claude Code's own internal
    # retries arrive as fresh dispatches and are counted here; `retries` (which
    # counts llm:attempt[n>0] spans) stays 0 on this engine because the harness
    # never tells the host it is retrying.
