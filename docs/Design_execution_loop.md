...
The orchestrator creates a fresh `AgentEngine` for each eligible request and dispatches that request onto a reusable execution-worker pool. A worker receives the internal request, copies the committed predecessor context into a working context, materializes resolved input dependencies, and only then lazily creates the agent sandbox and harness infrastructure. The caller sees only the request's pending `agdata` result.
...
An ordinary skill failure discards the working transaction and publishes a clean predecessor copy with the canonical rollback notice. Cancellation discards work without that notice.

## Safe-boundary controls

The internal request's cancellation callback and the agent's pause/redirect controls are propagated through `AgentEngine`, `HostServerManager`, the host interaction service, and native or external harness paths. They are observed only at explicit boundaries:

| Boundary | What has completed | Control behavior |
|---|---|---|
| Before infrastructure | Fresh engine and worker job exist; contexts are copied | Cancellation avoids sandbox and service creation |
| Before harness execution | Sandbox lock acquired | Pause may park; cancellation discards |
| Before model generation | Prior history/tool result is complete | FIFO redirects may be attached when the protocol permits a user turn |
| After model output / before tools | Provider call is complete | Cancellation is observed; a final answer atomically hands off in-flight redirects or establishes the closing fence |
| After each tool result is published | Tool call is complete | Pause/cancellation is observed before another model turn |
...
At a final model result, Agency atomically checks the request's redirect inbox. If a message was accepted while the provider call was in flight, the LLM service performs a follow-up generation with that message before it may close. Otherwise the request crosses its closing fence. Cancellation races with a monotonic completion claim: if cancellation wins, the container and working context are discarded; if completion wins, a later cancellation cannot retroactively invalidate the already claimed transaction.

## Native and external harnesses

The native ReAct loop checkpoints before generation, after internal compaction, after model output, and after each published tool result. Internal compaction requests explicitly bypass redirects, but a post-compaction checkpoint still observes pause and cancellation before task generation.

External harnesses send model traffic through the request-bound LLM service. The service fingerprints the semantic request to derive a stable boundary ID, admits redirects only where adding a user turn preserves tool-call pairing, and retains the assigned overlay by sequence and a separately derived history anchor. An identical provider or CLI retry reuses that boundary assignment instead of consuming a message twice. If external compaction removes an old anchor, an already admitted instruction moves to the current generation instead of disappearing. A redirect that arrives during a final provider call is assigned at an atomic post-model boundary and produces a traced follow-up generation.

Both paths use the same internal request control state. A harness cannot select another request by supplying an ID.
...
After execution returns, the worker posts a completion event; it does not publish futures itself. The scheduler event thread then settles the output-context future, finalizes request bookkeeping, releases the agent and global capacity slot, and only afterward settles the public result future. Callbacks therefore observe a settled context and may safely inspect history or submit follow-up work.
