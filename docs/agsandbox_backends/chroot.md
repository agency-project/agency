...
- `_kill_all_sandbox_processes()` — `os.killpg()` on every tracked group, called by `stop()`/`destroy()`/`restore()` before tearing down or overwriting the workspace. There is no container-removal-style implicit kill here (chroot has no cgroup/namespace boundary whose teardown guarantees termination), so this explicit step is the only thing that stops a background process from outliving the workspace being deleted or rewritten out from under it.
...
**GPU release**: there is no `_gpu_is_clear()`/polling mechanism on this backend (or any backend — the concept was removed entirely). `agResourcePool.release_gpu()` releases the semaphore immediately, with no wait. Safety comes from ordering: `stop()`/`destroy()` always call `_kill_all_sandbox_processes()` synchronously *before* releasing the GPU, so by the time release happens, everything this backend could see and terminate already has been. There is also no agent-facing `gpu_release` tool — once `reserve_gpu` triggers a real GPU acquisition, it's held for the rest of the sandbox's lifetime and freed automatically by `stop()`/`destroy()`, not by an explicit release call.
...
`stop(commit=True)` snapshots the workspace this way but **leaves the live workspace in place** — unlike the container backends, there's no container process that must be torn down to "stop," so the directory just keeps being reused directly by the next tool call, with no destroy-and-recreate round trip. `stop(commit=False)` deletes the live workspace outright; it does **not** eagerly restore it from the last checkpoint. That restore is deferred to the next `_ensure_started()` call, which already materializes from `_checkpoint_image` whenever it finds the workspace missing (see below) — doing the same copy at `stop()` time too would pay that cost even when the sandbox is never used again (e.g. right before a `destroy()` that deletes the whole jail root moments later), and would introduce a real failure surface (a `cp -a` that can raise) into a step that otherwise can't fail.
