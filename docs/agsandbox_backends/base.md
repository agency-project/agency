# Sandbox backend selection (`agsandbox_backends/base.py`)
...
`agSandbox` (in `agsandbox.py`) is a thin facade: it resolves the image/mounts vocabulary that's meaningful regardless of backend, then builds and delegates every operation to an `agsandbox_backend` (in `agsandbox_backends/base.py`) chosen by `agSandboxBackendConfig.backend`. Three concrete backends exist today, each a real subclass in its own module:
...
`"auto"` (`agsandbox_backends.base._auto_detect_runtime()`) probes in this order: **podman → docker → chroot**, first usable one wins.

- Podman/docker usability: the binary is on `PATH` and `<runtime> info` succeeds (`agsandbox_backends.container._runtime_works()`), cached for the process lifetime in `_RUNTIME`.
- Chroot usability (`agsandbox_backends.chroot.chroot_available()`, also cached): see [chroot.md](chroot.md) for the two-layered probe (sysctl + live `unshare`, with a `rootlesskit`-wrapped fallback).
