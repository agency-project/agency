...
`UserPromptSubmit` hook acknowledgment. Redirect sends Escape, waits for a
...
Codex, Grok Build, and OpenCode use the shared PTY attempt runner and register
...
# Linux, installed Codex/Grok/OpenCode; synthetic model, no API key:
AGENCY_TEST_EXTERNAL_PTY=1 pytest tests/harness/test_external_pty_live.py
```
