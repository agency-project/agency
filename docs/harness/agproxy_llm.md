# LLM gateway for harness-driven agents (`harness/agproxy_llm.py`)
...
```python
from agency.harness.agproxy_llm import agProxyLLM, get_shared_gateway
...
- `/v1/messages`'s translated-response path was verified against the real `claude` CLI (not just
  mocked) — see `tests/harness/agharness_backends/test_claude_code.py`'s `real_claude`-marked tests,
