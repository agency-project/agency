...
The sandbox is created lazily by `AgentEngine.execute()`. After each tool call — success or failure, regardless of `run_in_subprocess` — the sandbox is only *hibernated* (`sandbox.stop()`: `docker/podman stop`, container kept, never committed or removed), **unless** the tool call left background work still running (`sandbox._has_pending_background_work()`), in which case `stop()` is deferred to a later call rather than killing that work. The next tool call's `_ensure_started()` resumes the same hibernating container in place (`docker/podman start`) with no image or `run` involved at all. Checkpointing/reverting happens once per engine execution: `sandbox.commit()` on success or `sandbox.rm_container()` on failure, with the decision and teardown performed while the engine still holds `sandbox._lock`. `_lifecycle_tag()` always lowercases the Docker image name (Docker requires lowercase repository names).
...
- `AgentEngine.execute()` always provisions a sandbox when `ag.sandbox is None`, and always calls `sandbox.commit()`/`sandbox.rm_container()` at the execution boundary (success/failure respectively) — regardless of whether the sandbox was created by the engine or handed in via `agent(sandbox=...)` / direct assignment (`ag.sandbox = sb`). Neither call is destructive to the Python object itself: `commit()` doesn't touch the container's existence at all, and even `rm_container()` just means the next access transparently restarts it from the last checkpoint.
...
`agSandbox.__init__` allocates `self._lock = threading.RLock()` for this purpose. `AgentEngine.execute()` acquires it right after provisioning and releases it in `finally`, only after the final `commit()`/`rm_container()` and optional `stop()` have completed. Everything the execution does to the sandbox in between reentrantly re-acquires the same `RLock` for free.

This lock is **not** self-enforcing on `agSandbox` — calling `sandbox.exec()` directly does not itself acquire the lock. `AgentEngine.execute()` establishes the "one execution owns this sandbox at a time" invariant; code that drives a shared sandbox outside the engine is responsible for its own coordination. See `Design_sandbox_lifecycle.md`'s "Concurrency controls" section and `agsandbox.md` for details.
