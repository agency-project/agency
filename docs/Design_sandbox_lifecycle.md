...
|---|---|
| Success | `ag.sandbox.commit()` — checkpoints the container's current filesystem into its lifecycle image (`agency/lifecycle-<name>`), **without removing or even stopping the container**. The very next skill call (on this agent, or whichever one next holds this `agSandbox`) resumes directly from the same container object — no `docker/podman run` needed either way. |
