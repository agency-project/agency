...
def test_run_returns_pending_agdata():
    ...
    skill.execute_engine = fake_execute_react
...
def test_run_dispatches_to_execute_engine_when_native():
    """Every configured harness is reached through the agent driver engine."""
    calls = []

    def fake_execute_engine(ag, prev_ctx, inp, max_steps=None, **_):
        calls.append("engine")
        return agdata(done=True), prev_ctx, []

    skill = agskill(name="s", system_prompt="")
    skill.execute_engine = fake_execute_engine
...
def test_run_dispatches_to_execute_engine_when_engine_not_native():
    calls = []
...
def test_multiple_agskills_coexist():
    ...
    skill_a.execute_engine = fake_a
    skill_b.execute_engine = fake_b
...
def test_save_and_load_restores_history_and_filesystem(tmp_path, monkeypatch):
    ...
    skill_write.execute_engine = fake_write
