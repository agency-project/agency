...
(agency.sandbox.podman._PodmanBackend): dangling-image cleanup on
...
installed (see sandbox.base.agsandbox_backend.for_config()) and
the one actually subject to the exact same session-keyring quota as Docker
(see sandbox/container.py's module docstring).
...
class TestHostToContainerId:
    ...
    def test_translate_id_leaves_unmapped_host_id_unchanged(self):
        from agency.sandbox.podman import _translate_id
