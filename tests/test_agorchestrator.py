...
def _agent(tmp_path, *, max_engines=None, db_path=None):
    config = agConfig(
        agOrchestratorConfig(
            max_concurrent_engines=max_engines,
            db_path=str(db_path or (tmp_path / "agency.sqlite3")),
...
def test_orchestrator_package_layout(tmp_path):
    ...
    orchestrator = get_orchestrator(
        ag.agconfig,
        default_db_path=tmp_path / "agency.sqlite3",
    )
...
def test_shutdown_flushes_database_and_rejects_new_submissions(monkeypatch, tmp_path):
    def execute(self, *, context, **_kwargs):
        return _result(context, ok=True)
    ...
    orchestrator = get_orchestrator()
    orchestrator.data_collector.subscribe(lambda _record: orchestrator.snapshot())
    orchestrator.shutdown(timeout_s=2)

    assert (tmp_path / "agency.sqlite3").exists()
...
def test_profiler_adapter_persists_correlated_request_intervals(monkeypatch, tmp_path):
    dependency: Future[agdata] = Future()
    ...
    connection = sqlite3.connect(tmp_path / "agency.sqlite3")
    try:
        rows = connection.execute(
            "SELECT name,agname,request_id,skill FROM spans "
            "WHERE source='profiler_adapter' ORDER BY id"
        ).fetchall()
    finally:
        connection.close()
    ...
    assert all(row[1:] == (str(ag.agname), "run0", "profiled") for row in rows)


def test_database_failure_is_telemetry_only(monkeypatch, tmp_path):
    def execute(self, *, context, **_kwargs):
        return _result(context, ok=True)

    monkeypatch.setattr(AgentEngine, "execute", execute)
    ag = _agent(tmp_path, db_path=tmp_path)

    assert ag.run(agskill("s", ""), agdata()).ok is True
    snapshot = get_orchestrator().snapshot()
    assert snapshot.persistence_error is not None
