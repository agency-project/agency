...
def test_run_returns_agdata_and_history():
    ...
    with patch("openai.OpenAI") as MockClient:
        ...
        result, hist = s.run(LLM_CONFIG, agdata(text="hello"), agdata(messages=[]), [])
...
def test_run_direct_json_response():
    ...
    with patch("openai.OpenAI") as MockClient:
        ...
        result, _ = s.run(LLM_CONFIG, agdata(q="6*7"), agdata(messages=[]), [])
...
def test_run_plain_text_fallback():
    ...
    with patch("openai.OpenAI") as MockClient:
        ...
        result, _ = s.run(LLM_CONFIG, agdata(q="hi"), agdata(messages=[]), [])
...
def test_system_prompt_not_in_returned_history():
    ...
    with patch("openai.OpenAI") as MockClient:
        ...
        _, hist = s.run(LLM_CONFIG, agdata(x=1), agdata(messages=[]), [])
...
def test_tool_call_executes_and_continues():
    ...
    t = tool(name="calc", description="", fn=fn,
    ...
    with patch("openai.OpenAI") as MockClient:
        ...
        result, hist = s.run(LLM_CONFIG, agdata(task="calc"), agdata(messages=[]), [])
...
def test_max_steps_exceeded():
    ...
    with patch("openai.OpenAI") as MockClient:
        ...
        result, _ = s.run(LLM_CONFIG, agdata(x=1), agdata(messages=[]), [], max_steps=3)
...
def test_tools_none_inherits_agent_tools():
    ...
    agent_tool = tool(name="at", description="agent tool", fn=lambda a: agdata(r=1))
...
def test_tools_empty_list_overrides_agent_tools():
    ...
    agent_tool = tool(name="at", description="", fn=lambda a: agdata())
...
def test_input_schema_missing_field_returns_error():
    ...
    result, _ = s.run(LLM_CONFIG, agdata(question="hi"), agdata(messages=[]), [])
...
def test_input_schema_type_error_returns_error():
    ...
    result, _ = s.run(LLM_CONFIG, agdata(count="not-an-int"), agdata(messages=[]), [])
    assert result.error is not None
...
def test_input_schema_valid_proceeds():
    ...
    with patch("openai.OpenAI") as MockClient:
        ...
        result, _ = s.run(LLM_CONFIG, agdata(text="hello"), agdata(messages=[]), [])
...
def test_input_schema_description_value_only_checks_presence():
    ...
    with patch("openai.OpenAI") as MockClient:
        MockClient.return_value.chat.completions.create.return_value = _direct("{}")
        result, _ = s.run(LLM_CONFIG, agdata(query=42), agdata(messages=[]), [])
...
def test_output_schema_missing_field_triggers_retry():
    ...
    with patch("openai.OpenAI") as MockClient:
        MockClient.return_value.chat.completions.create.side_effect = responses
        result, _ = s.run(LLM_CONFIG, agdata(text="hi"), agdata(messages=[]), [])
...
def test_output_schema_retry_exhausted_returns_error():
    ...
    with patch("openai.OpenAI") as MockClient:
        ...
        result, _ = s.run(LLM_CONFIG, agdata(q="hi"), agdata(messages=[]), [], max_steps=10)
    assert result.error is not None
...
def test_output_schema_type_mismatch_triggers_retry():
    ...
    with patch("openai.OpenAI") as MockClient:
        MockClient.return_value.chat.completions.create.side_effect = responses
        result, _ = s.run(LLM_CONFIG, agdata(x=1), agdata(messages=[]), [])
