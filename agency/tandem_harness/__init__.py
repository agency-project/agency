"""Standalone tandem harness -- a self-contained ReAct coding-agent CLI,
independent of agency's host-side runtime.

**Invocation -- deliberately NOT `python3 -m agency.tandem_harness`:**
...
package AS `agency.tandem_harness` (via `-m agency.tandem_harness` or
`import agency.tandem_harness`) would trigger that whole chain regardless
of anything this package's own code does.

The fix: run it with `agency/` itself (not the repo root) on `PYTHONPATH`,
so `tandem_harness` resolves as its OWN top-level package -- a sibling of
`agency/__init__.py`, never an ancestor of it:

    PYTHONPATH=<repo>/agency python3 -m tandem_harness.cli -p "<prompt>" --model <name> ...
...
`anthropic`/`boto3` -- only `tandem_harness` and whatever
