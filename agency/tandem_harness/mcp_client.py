"""MCP client for the standalone tandem harness's extension tools.
...
so a real, standalone MCP server works identically whether tandem_harness
    ...
    def discover(self) -> "list[dict]":
        ...
        for _name, url, headers in self._servers:
            try:
                ...
                print(f"[tandem_harness] WARNING: MCP server {url!r} unreachable: {e}")
                continue
            for schema in tools:
