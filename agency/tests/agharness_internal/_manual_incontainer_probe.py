...
def run_probe(argv, expect_stdout_contains=None, expect_returncode=0):
    ...
    spec = {"argv": argv, "envp": {"PATH": "/usr/bin:/bin"}, "cwd": "", "syscalls": ["execve", "execveat"]}
    ...
    if expect_stdout_contains:
        assert expect_stdout_contains in result["stdout"], f"stdout missing {expect_stdout_contains!r}: {result['stdout']!r}"
...
if __name__ == "__main__":
    run_probe(["/bin/sh", "-c", "echo hello-from-traced-child && /bin/true"], expect_stdout_contains="hello-from-traced-child")
    print()
