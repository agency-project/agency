...
def install_claude(sandbox):
    ...
    subprocess.run([runtime, "cp", CLAUDE_HOST_BINARY, f"{name}:/usr/local/bin/claude"], capture_output=True)
...
def main():
    ...
    try:
        ...
        r1 = ag.run(skill, agdata(instruction=(
            "My project's deployment codename is PURPLE-42-NARWHAL. "
            "Just say OK."
        )))
        r1._resolve()
    ...
    try:
        ...
        r2 = ag.run(skill, agdata(instruction="What's my project's deployment codename? Reply with just the codename."))
        r2._resolve()
        print("call 2 result:", r2._data)

        assert not r2.is_pending() and not r2._data.get("error")
        text = r2._data.get("result", "")
        assert "PURPLE-42-NARWHAL" in text, f"continuity FAILED -- agent didn't recall the code: {text!r}"
