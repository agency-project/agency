...
def run_probe(
    argv, decide=lambda ev: "allow", expect_returncode=None, expect_stdout_contains=None, label=""
):
    ...
    if expect_stdout_contains:
        assert expect_stdout_contains in result["stdout"], (
            f"missing {expect_stdout_contains!r} in {result['stdout']!r}"
        )
    print("  PASS")
...
if __name__ == "__main__":
    ...
    run_probe(
        [
            "/bin/sh",
            "-c",
            "echo IN_CONTAINER_PID=$$ && pwd && echo hi > /workspace/probe_marker.txt",
        ],
        expect_returncode=0,
        expect_stdout_contains="IN_CONTAINER_PID=",
        label="allow-baseline",
    )
    ...
    check = subprocess.run(
        ["docker", "exec", CONTAINER, "cat", "/workspace/probe_marker.txt"],
        capture_output=True,
        text=True,
    )
    assert check.stdout.strip() == "hi", f"marker file not found/wrong in container: {check!r}"
    print(
        "[filesystem-check] /workspace/probe_marker.txt correctly written inside the container: PASS"
    )
    ...
    run_probe(
        ...
        decide=deny_true,
        expect_stdout_contains="exit_was=",
        label="deny-specific-exec",
    )
