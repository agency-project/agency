...
def _make_stream(content: str):
    """Minimal streaming mock returning a single content chunk then a usage chunk."""
    class _Delta:
        def __init__(self, c):
            self.content = c
...
def test_writer_skill_run_missing_required_input_fields():
    s = WriterSkill()
    # Missing both fields — input validation should reject immediately
...
def test_writer_skill_run_output_missing_field_triggers_retry():
    ...
    responses = iter([
        _make_stream('{"path": "/tmp/f.txt"}'),          # missing "status" → retry
...
def test_writer_skill_history_grows_with_messages():
    ...
    with patch("openai.OpenAI") as M:
        ...
        _, hist, delta = s.run(_LLM, agdata(file_path="/tmp/f.txt", content="hi"),
                               agdata(messages=[]), [])
    # At minimum: user message + assistant message appended
