...
def _fetch_paper_fn(arg: agdata) -> agdata:
    url = str(arg.url)  # type: ignore[arg-type]
    ...
    # Trim boilerplate: skip lines before the first heading
...
def test_fetch_paper_fn_returns_text_and_correct_url(monkeypatch):
    html = "<html><body><h1>Introduction</h1><p>This paper proposes X.</p></body></html>"
    _mock_http(monkeypatch, html)
    result = _fetch_paper_fn(agdata(url="https://arxiv.org/abs/2301.12345"))
    assert len(result.text) > 0
    assert result.url == "https://arxiv.org/html/2301.12345"
    assert result.truncated is False


def test_fetch_paper_fn_skips_lines_before_first_heading(monkeypatch):
    html = (
        "<html><body>"
...
def test_fetch_paper_fn_truncates_and_flags_long_content(monkeypatch):
...
def test_fetch_paper_fn_short_content_not_flagged_truncated(monkeypatch):
...
def test_fetch_paper_fn_handles_errors(monkeypatch, exc_type, exc_args):
    def _raise(*a, **kw):
