...
class agskill:
    ...
    def run(
        ...
        try:
            ...
            active_emitter = (
                _agwebui._active.emitter if _agwebui._active is not None else None
            )
        except Exception:
            active_emitter = None
        default_db_path = (
            active_emitter._db_path
            if active_emitter is not None
            else ag.data_collector.db_path
        )
        orchestrator = get_orchestrator(
            ag.agconfig,
            default_db_path=default_db_path,
        )
        ag.data_collector.bind_collector(orchestrator.data_collector)
