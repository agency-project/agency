...
A multi-agent framework with sandboxed execution, isolated filesystems, GPU access control, and automatic background-process tracking. Each agent execution resolves or creates a durable `agSandbox` facade, acquires its lease, and explicitly prepares the selected physical backend before host services or the sandbox-side harness start. Successful executions checkpoint the sandbox and may hibernate it; failed executions discard dirty live state while preserving the last successful checkpoint. Permanent `destroy()` remains a separate sandbox-lifetime operation.
...
**`agent`** — a pure state container: holds an LLM config, sandboxed tools, conversation context (`agcontext`), and a name. It does not own an execution loop. `agent.run(skill, input)` is a thin dispatch call; the scheduling wrapper is `agskill.run()`, which spawns a daemon thread and routes the synchronous transaction through `agentEngine` and `ExecutionBuilder`. The selected sandbox-side harness owns its own execution loop. Each `run()` call is non-blocking and returns a pending `agdata` that resolves lazily. Sequential calls on the same agent are automatically serialised through the history chain. The first execution attaches an `agSandbox` facade when the agent does not already have one; that facade and its last checkpoint remain available between tasks even when the physical backend is hibernating or its dirty live state has been discarded. Forking via `agent(parent)` deep-copies the context and copies the parent's checkpoint (via `docker tag`, Podman's equivalent, or a directory copy for a chroot-backed sandbox — see [agsandbox.md](docs/agsandbox.md)); the fork's physical backend is explicitly prepared by its next execution.

**`agskill`** — a named skill with its own system prompt, optional input/output schemas, and an optional tool list. `agskill.run()` is a non-blocking scheduling wrapper: it spawns a daemon thread and returns a pending `agdata` immediately, then delegates synchronous execution to `agentEngine`.

**`agentEngine`** — the thin public execution facade and composition root. Its only execution method is `execute()`: it constructs or receives the builder and its dependencies, then delegates the complete transaction to `ExecutionBuilder.execute()`.

**`ExecutionBuilder`** — sequences one execution transaction: acquire a provisioned sandbox lease, start `HostServerManager` and obtain its real UDS path, build the prompt, mark that execution is about to be attempted, launch/run/wait for the sandbox-side harness, stop the harness and host services, and finalize the lease.

**`SandboxProvisioner`** — owns the sandbox transaction boundary. It resolves or creates and attaches the `agSandbox` facade without mutating the agent's original config, acquires `sandbox._lock`, explicitly calls the backend-neutral `sandbox.ensure_started()`, commits or discards, optionally hibernates, queues a revert notice after attempted failures, tears down provisioner state, and releases the lock last.

**`agSandbox` and its backends** — provide the backend-neutral isolation mechanics for Docker, Podman, and chroot. Normal execution finalization preserves the durable facade/checkpoint; `destroy()` is permanent lifetime cleanup.

**`agtool`** — a named callable an LLM can invoke via function calling. Every tool call is offloaded to a `ProcessPoolExecutor` worker so CPU-bound tools don't block other agents. Tools are serialised with `cloudpickle`, so bound methods work without any extra machinery. A tool failure does not independently checkpoint or revert the workspace; the completed execution outcome determines whether `SandboxProvisioner` commits or discards the transaction. Agents can pass `"timeout": <seconds>` in any tool call's arguments to override the default 30 s watchdog.
...
| [agskill.md](docs/agskill.md) | `run()` scheduling wrapper, execute-only engine handoff, schemas, typed fields, and the retired in-process ReAct loop (historical) |
