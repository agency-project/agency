...
**`GlobalAgentOrchestrator`** — the lazy process-wide scheduler, dependency resolver, wait queue, and global data collector. Submission, dependency completion, and engine completion push events to its scheduler thread; each event runs a complete resolve-all-waiting → promote-all-ready → schedule-ready cycle, with no completion polling. Configure it with `agOrchestratorConfig(max_concurrent_engines=..., db_path=...)`, and inspect or drain it through `get_orchestrator().snapshot()`, `.flush()`, and `.shutdown()`.
...
**`agwebui`** — a browser-based dashboard that runs in a separate process. Its emitter is a scoped publisher into the global asynchronous SQLite writer; a standalone FastAPI server reads the same database's UI projections and pushes updates to connected browsers over WebSocket. Command and human-reply channels remain file based. See [docs/agwebui.md](docs/agwebui.md).
...
| [Design_orchestrator.md](docs/Design_orchestrator.md) | Global scheduler, dependency futures, queue policy, collector, and shutdown |
